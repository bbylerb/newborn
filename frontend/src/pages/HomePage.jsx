...
export default function HomePage() {
  ...
  const iconBg = { red: '#fa98a6', gold: '#fa98a6', blue: '#fa98a6', green: '#fa98a6' };
      ...
      <div style={{ background: 'linear-gradient(135deg,#C8102E,#9B0C23)', padding: '0 22px 22px', flexShrink: 0, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0 16px', position: 'relative' }}>
          <div>
            ...
            <div style={{ color: '#fff', fontSize: 20, fontWeight: 800 }}>
              {user?.full_name?.split(' ')[0]} {user?.full_name?.split(' ')[1]?.[0] ? user.full_name.split(' ')[1][0] + '.' : ''}
            </div>
          </div>
          <button
            onClick={() => navigate('/notifications')}
            style={{ width: 40, height: 40, background: 'rgba(255,255,255,.15)', borderRadius: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, position: 'relative', cursor: 'pointer', border: 'none', flexShrink: 0 }}
          >
            🔔
            {unread > 0 && (
              <span style={{ position: 'absolute', top: 8, right: 8, width: 8, height: 8, background: '#F5A623', borderRadius: '50%', border: '2px solid #C8102E' }} />
            )}
          </button>
        </div>
        ...
        {announcements[0] ? (
          <div style={{ background: 'rgba(255,255,255,.14)', borderRadius: 13, padding: '12px 14px', border: '1px solid rgba(255,255,255,.2)', display: 'flex', gap: 10 }}>
            ...
            <div>
              <div style={{ display: 'inline-block', background: '#F5A623', color: '#7c3a00', fontSize: 10, fontWeight: 800, padding: '2px 8px', borderRadius: 50, marginBottom: 4 }}>
                {t('home.announcementLabel')}
              </div>
              <div style={{ color: '#fff', fontSize: 13, lineHeight: 1.4 }}>
                {t('lang') === 'th' ? announcements[0].body_th : announcements[0].body_en}
              </div>
            </div>
          </div>
        ) : (
      ...
      <div className="scrl">
        <div style={{ padding: '18px 18px 20px' }}>
          ...
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--t1)' }}>{t('home.services')}</div>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            ...
            <button onClick={() => navigate('/parcels')} style={{ fontSize: 13, color: 'var(--r)', fontWeight: 700, background: 'none', border: 'none', cursor: 'pointer' }}>
              {t('common.seeAll')}
            </button>
          </div>
          ...
          <div className="card" style={{ padding: '4px 14px 6px', marginBottom: 14 }}>
            {parcels.length === 0 ? (
              ...
              >
                <div style={{ width: 36, height: 36, borderRadius: 10, background: '#FEE2E6', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 17, flexShrink: 0 }}>
                  📮
                </div>
                <div style={{ flex: 1 }}>
          ...
          <button
            ...
            style={{ width: '100%', background: 'linear-gradient(135deg,#1e3a5f,#111827)', borderRadius: 'var(--rad)', padding: 16, display: 'flex', alignItems: 'center', gap: 14, cursor: 'pointer', border: 'none', textAlign: 'left' }}
          >
            <div style={{ width: 44, height: 44, borderRadius: 13, background: 'rgba(255,255,255,.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>
              📋
            </div>
            <div style={{ flex: 1 }}>
